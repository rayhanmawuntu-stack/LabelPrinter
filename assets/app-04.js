...
function renderHistory(){
  ...
  $('historyKpis').innerHTML=[['Saved batches',history.length,'primary'],['Total labels',total,'accent'],['Recipients',unique,'soft'],['Latest layout',history[0]?LAYOUTS[history[0].layout]?.label||history[0].layout:'—','neutral']].map(x=>`<article class="metric ${x[2]}"><span>${x[0]}</span><strong>${esc(x[1])}</strong><small>${connected?'Google Sheets + cache':'Local cache'}</small></article>`).join('');
