(()=>{
  ...
  function normalizedHistory(){
    ...
    return{batches:[...byId.values()],invalidTimestamps,missingBatchIds,emptyBatches,duplicateBatchIds,duplicateRecords:duplicateBatchIds.reduce((sum,[,count])=>sum+count-1,0)};
  }
  ...
  function annualReportData(year){
    ...
    all.forEach(batch=>batch.labels.forEach(row=>{
      const recipient=full(row)||clean(row.company)||'Unknown recipient';
      const current=customerFirstSeen.get(recipient);
      if(!current||batch._date<current)customerFirstSeen.set(recipient,batch._date);
    }));
    ...
    selected.forEach(batch=>{
      const month=months[batch._date.getMonth()],user=clean(batch.user||batch.nickname)||'Unknown user',state=syncState(batch.syncState),sheetCount=estimatedSheets(batch);
      month.labels+=batch.labels.length;month.batches++;month.sheets+=sheetCount;
      increment(month.users,user,batch.labels.length);increment(month.layouts,LAYOUTS[batch.layout]?.label||clean(batch.layout)||'Unknown',1);increment(syncCounts,state,1);
      ...
      batch.labels.forEach(row=>{
        ...
        month.recipients.add(recipient);increment(month.customers,recipient,1);increment(month.senders,sender,1);userRecord.customers.add(recipient);

        let customerRecord=customers.get(recipient);
        if(!customerRecord){customerRecord={prefix,company,labels:0,batches:new Set(),months:new Set(),first:customerFirstSeen.get(recipient)||batch._date,last:0,senders:new Map(),users:new Map(),contacts:new Set(),addresses:new Set()};customers.set(recipient,customerRecord)}
        customerRecord.prefix=prefix||customerRecord.prefix;customerRecord.company=company||customerRecord.company;customerRecord.labels++;customerRecord.batches.add(batch.id);customerRecord.months.add(batch._date.getMonth());customerRecord.last=Math.max(customerRecord.last,batch._date.getTime());increment(customerRecord.senders,sender,1);increment(customerRecord.users,user,1);if(clean(row.attn))customerRecord.contacts.add(clean(row.attn));if(clean(row.address))customerRecord.addresses.add(clean(row.address));
        ...
        senderRecord.labels++;senderRecord.batches.add(batch.id);increment(senderRecord.customers,recipient,1);senderRecord.months.add(batch._date.getMonth());senderRecord.last=Math.max(senderRecord.last,batch._date.getTime());
      });
    });

    const firstDate=selected.length?selected[selected.length-1]._date:null,lastDate=selected.length?selected[0]._date:null;
    const totals={labels:selected.reduce((sum,batch)=>sum+batch.labels.length,0),batches:selected.length,sheets:selected.reduce((sum,batch)=>sum+estimatedSheets(batch),0),recipients:customers.size,users:users.size,senders:senders.size,activeMonths:months.filter(month=>month.batches).length,synced:syncCounts.get('synced')||0,pending:syncCounts.get('pending')||0,failed:syncCounts.get('failed')||0,previousLabels:previousLabels.reduce((sum,value)=>sum+value,0),firstDate,lastDate};
    ...
    return{...source,selected,previousLabels,months,customers,users,senders,layouts,totals,customerFirstSeen};
  }
  ...
  function buildWorkbook(year){
    ...
    report.months.forEach(month=>{
      ...
      const topCustomer=topEntry(month.customers),topUser=topEntry(month.users),topSender=topEntry(month.senders),topLayout=topEntry(month.layouts);
      const newRecipients=[...month.recipients].filter(recipient=>{const first=report.customerFirstSeen.get(recipient);return first&&first.getFullYear()===year&&first.getMonth()===month.month}).length;
  ...
  function saveWorkbook(book,fileName){
    ...
    anchor.href=url;anchor.download=fileName;anchor.style.display='none';document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
  }

  async function downloadMonthlyReport(){
    if(button.disabled)return;
    const year=Number(document.getElementById('monthlyYear')?.value)||ensureMonthlyYear(),original=button.textContent;
